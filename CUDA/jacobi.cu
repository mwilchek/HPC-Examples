...
void print_matrix_sample(double * __restrict M,int ROWS, int COLS)
{
	int i, j;
	for (i = 10; i >= 0; i--)
	{
...
void init_grid_U(double * __restrict grid, double * __restrict grid_prev, int ROWS, int COLS)
{
#pragma omp parallel
        {
        #pragma omp single
...
__global__ void compute_gridpoints_GPU(double * __restrict grid, double * __restrict grid_prev, const int ROWS, const int COLS)
{
