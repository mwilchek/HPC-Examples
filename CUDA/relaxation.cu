...
void init_grid_CPU(double * grid, double * grid_prev, int ROWS, int COLS)
{
#pragma omp parallel
	{ 
	#pragma omp single
...
__global__ void jacobi_iterator_GPU(const double * grid, double * grid_prev, const int ROWS, const int COLS)
{
	...
	{
		grid_prev[j * COLS + i] = (grid[j * COLS + (i - 1)] + grid[j * COLS + (i + 1)] + grid[(j - 1) * COLS + i] + grid[(j + 1) * COLS + i] ) * 0.25;
...
int main()
{
	...
	const int ROWS         = 20+2;                           // --- Grid size is N x N
	const int COLS         = 20+2;                           // --- Grid size is N x N
	...
	init_grid_CPU(h_grid, h_grid_prev, ROWS, COLS);
	gettimeofday(&stop_time_CPU,NULL);
	print_matrix(h_grid,ROWS,COLS);
	...
	{
		jacobi_iterator_GPU<<<DimGrid, DimBlock>>>(d_grid, d_grid_prev, ROWS,COLS);
